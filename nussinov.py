...
# This code was modified for task 5
...
def pair_check(tup):
    if tup in [('A', 'U'), ('U', 'A'), ('C', 'G'), ('G', 'C'), ('U', 'A')]:  # modified for wobbly pair G - U
        return True
    return False


def fixed_score(i, j, sequence):  # added function that gives fixed score values for all pairs
...
def OPT(i, j, sequence):  # modified for fixed value of scores
    """
    ...
    if i >= j - min_loop_length:
        ...
        unpaired = OPT(i, j - 1, sequence) + fixed_score(i, j, sequence)
...
def traceback(i, j, structure, DP, sequence):  # modified for fixed score
    ...
    # consider cases where j forms a pair.
        ...
        for k in [b for b in range(i, j - min_loop_length) if pair_check((sequence[b], sequence[j]))]:
            ...
            if k - 1 < 0:
                if DP[i][j] == DP[k + 1][j - 1] + 1 + fixed_score(i, j, sequence):
                    structure.append((k, j))
                    traceback(k + 1, j - 1, structure, DP, sequence)
                    break
            elif DP[i][j] == DP[i][k - 1] + DP[k + 1][j - 1] + fixed_score(i, j, sequence):
